/*
 * MockFrame2TestCase - a JUnit extension for testing Frame2 events and
 * event handlers in the context of the HttpFrontController.
 *
 * Based on StrutsTestCase by Deryl Sealef
 *
 * This library is free software; you can redistribute it and/or
 * modify it under the terms of the Apache Software License as
 * published by the Apache Software Foundation; either version
 * 1.1 of the License, or (at your option) any later version.
 *
 * This library is distributed in the hope that it will be
 * useful, but WITHOUT ANY WARRANTY; without even the implied
 * warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR
 * PURPOSE.  See the Apache Software Foundation Licens for more
 * details.
 *
...
/**
 * MockFrame2TestCase is an extension of the base JUnit testcase that
 * provides additional methods to aid in testing WAM events and event
 * handlers.  It uses a mock object approach to simulate a servlet
 * container, and tests the execution of events as they
 * are actually run through the WAM HttpFrontController.  WamTestCase
 * provides methods that set up the request path, request parameters
 * for event subclasses, as well as methods that can verify
...
public class MockFrame2TestCase extends TestCase {
   ...
   ServletContextListener _listener;
	...
	protected void setUp() throws Exception {
      ...
      LoggerFactory.setType("org.megatome.frame2.log.impl.Log4jLogger", getClass().getClassLoader());
		...
		//_request = new HttpServletRequestSimulator(_config.getServletContext());
		_request = new Frame2HttpServletRequestSimulator(_config.getServletContext());
		_response = new HttpServletResponseSimulator();
		_context = (MockFrame2ServletContextSimulator) _config.getServletContext();
   ...
   public void sendContextInitializedEvent(String key1,String value1,String key2,String value2) {
   ...
   public HttpFrontController getServlet() {
	...
	public void doEvent() {
		...
		try {
			...
			fail(
				"Error running doEvent(): "
					+ se.getRootCause().getClass()
					+ " - "
   ...
   public void setServletPath(String pathInfo) {
	...
	private String getActualForward() {
		if (_response.containsHeader("Location")) {
			...
			try {
				String strippedForward =
					_request.getContextPath()
						+ stripJSessionID(
							((ServletContextSimulator) _config.getServletContext())
	...
	public void verifyForwardPath(String forwardPath) throws AssertionFailedError {
		...
		if (!(actualForward.equals(forwardPath)))
			throw new AssertionFailedError(
				"was expecting '" + forwardPath + "' but received '" + actualForward + "'");
	}
	...
	// 		Common.setActionForm(form, request, eventPath, context, this.getActionServlet());
	// 		if (logger.isDebugEnabled())
	...
	//			forwardName = getActionConfig(servlet, actionPath, request, context).getInput();
	//			if (forwardName == null)
